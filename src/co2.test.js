...
import { MILLION, SWDV3, SWDV4 } from "./constants/test-constants.js";
import { GIGABYTE } from "./constants/file-size.js";

import CO2 from "./co2.js";
import { averageIntensity, marginalIntensity } from "./index.js";
import { SWDV4 as SWDV4Constants } from "./constants/index.js";

const TwnGridIntensityValue = averageIntensity.data["TWN"];
const SWDM4_GLOBAL_GRID_INTENSITY = SWDV4Constants.GLOBAL_GRID_INTENSITY;
...
describe("co2", () => {
  ...
  describe("Sustainable Web Design model as simple option", () => {
    ...
    const { PERBYTE_EMISSIONS_GB, PERBYTE_EMISSIONS_GB_GREEN } = SWDV4;
    ...
    describe("perVisit", () => {
      it("returns a CO2 number for data transfer per visit with caching assumptions from the Sustainable Web Design model", () => {
        co2.perVisit(GIGABYTE);
        expect(parseFloat(co2.perVisit(GIGABYTE).toFixed(5))).toBeCloseTo(
          parseFloat(PERBYTE_EMISSIONS_GB)
        );
      });

      it("returns a lower CO2 number for data transfer from domains using entirely 'green' power", () => {
        expect(
          parseFloat(co2.perVisit(GIGABYTE, false).toFixed(5))
        ).toBeCloseTo(parseFloat(PERBYTE_EMISSIONS_GB.toFixed(5)), 3);

        expect(parseFloat(co2.perVisit(GIGABYTE, true).toFixed(5))).toBeCloseTo(
          parseFloat(PERBYTE_EMISSIONS_GB_GREEN.toFixed(5)),
          3
        );
      });
    });
    ...
    describe("Returning results by segment", () => {
      ...
      describe("perVisit", () => {
        it("returns an object with devices, networks, data centers, and production emissions shown separately, as well as the total emissions", () => {
          co2 = new CO2({ model: "swd", version: 3, results: "segment" });
  ...
  describe("Error checking", () => {
    ...
    it("throws an error if model is not valid", () => {
      expect(() => (co2 = new CO2({ model: "1direction" }))).toThrow(
    ...
    it("throws an error if a version is present but no model is set", () => {
      expect(() => {
        co2 = new CO2({ version: 3 });
      }).toThrow(
        `Specified version 3 but an estimation model is missing. Please specify a model to use for the version you have set.`
      );
    });
  ...
  describe("Using values equal to 0", () => {
    ...
    it("expects perVisitTrace segments to be 0 when grid intensity is 0", () => {
      ...
      // Less than 0.1 because there's still a small production component that's calculated
      expect(co2Result.total).toBeLessThan(0.1);
      expect(co2Result["firstVisitCO2e"]).toBeLessThan(0.1);
      expect(co2Result["returnVisitCO2e"]).toBe(0);
    });
    ...
    it("expects perVisitTrace subsequent segments to be 0 when returning visitors is 0", () => {
      ...
      const co2Result = perVisitTraceResult.co2;
      expect(co2Result["firstVisitCO2e"]).toBeGreaterThan(0);
      expect(co2Result["returnVisitCO2e"]).toBe(0);
    });
  ...
  describe("Returning SWD results with rating", () => {
    ...
    it("returns a rating when rating is true and results are segmented", () => {
      ...
      expect(co2RatingSegmented.perByte(MILLION)).toHaveProperty("total");
    });
  });

  describe("Switch versions of the Sustainable Web Design model", () => {
    const co2 = new CO2({ model: "swd" });
    const co2SWDV3 = new CO2({ model: "swd", version: 3 });

    it("uses the SWD model version 4 by default", () => {
      expect(co2.model.version).toBe(4);
    });

    it("uses the SWD model version 3 when specified", () => {
      expect(co2SWDV3.model.version).toBe(3);
    });
  });
