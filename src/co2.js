...
function getCO2PerDomain(pageXray, greenDomains) {
  ...
  for (let domain of Object.keys(pageXray.domains)) {
    ...
    if (greenDomains && greenDomains.indexOf(domain) > -1) {
      Co2ForTransfer = getCO2PerByte(
        pageXray.domains[domain].transferSize,
        true
      );
    } else {
      Co2ForTransfer = getCO2PerByte(pageXray.domains[domain].transferSize);
    }
    co2PerDomain[domain] = Co2ForTransfer;
  }
  return co2PerDomain;
}
...
function getCO2perPage(pageXray, green) {
  ...
  let totalCO2 = 0;
  for (let domain of Object.keys(pageXray.domains)) {
    totalCO2 += domainCO2[domain];
  }
  return totalCO2;
}

/*
function perContentType(pageXray, green) {
  // return total co2 from all the resources of a given type, like images, movies etc.
}
...
function dirtiestResources(pageXray, greenDomains) {
  ...
  for (let asset of pageXray.assets) {
    ...
    allAssets.push({ url: asset.url, co2: co2ForTransfer });
  }
